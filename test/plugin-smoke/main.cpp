...
int main(int argc, char *argv[])
{
    ...
    // A configured one-word engine must reject a two-word sampling frame.
    char wrongWordCount[] = {
        char(0x34), char(0x12), char(0x56), char(0xA5),
        char(0x01), char(0x02), char(0x03), char(0x04),
        char(0x00), char(0x00), char(0x80), char(0x7F)
    };
    engine->ProcessingDatas(wrongWordCount, int(sizeof(wrongWordCount)));
    const QList<Frame> invalidFrames = engine->frame_list();
    if (invalidFrames.size() != 1 || invalidFrames.first().is_valid_)
        return fail(QStringLiteral("wrong word count was not rejected"), 11);
